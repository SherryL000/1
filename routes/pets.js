...
router.get('/world', async (req, res) => {
  try {
    ...
    console.log(`Found ${pets.length} pets for user ${req.session.userId}`); // Debug line

    res.render('pets/world', {
      title: 'Pet World',
      pets: pets, // This must be passed
      dbStatus: 'Connected',
      error: null
    });
  } catch (error) {
    console.error('Error loading pet world:', error);
    res.render('pets/world', {
      title: 'Pet World',
      pets: [],
      error: 'Failed to load pet world',
      dbStatus: 'Connected'
    });
  }
});
